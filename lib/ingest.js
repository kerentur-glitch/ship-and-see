...
function summarize(cleanedEvents) {
  ...
  for (const event of cleanedEvents) {
    ...
    if (event.type === "reaction") {
      if (event.isBot) {
        ...
        // Same person, same reaction, twice (e.g. a refresh-and-relike) —
        // counts once. A reload gets a fresh session_id but the same
        // (ip_trunc, ua) identity, so this catches it where session_id can't.
        const reactionKey = `${identity}|${event.reaction}`;
        if (page._reactedKeys.has(reactionKey)) {
          page.duplicateReactionsFiltered += 1;
        } else {
          page._reactedKeys.add(reactionKey);
  ...
  for (const [pageId, page] of Object.entries(pages)) {
    result[pageId] = {
      ...
      duplicateReactionsFiltered: page.duplicateReactionsFiltered,
    };
  }
  return result;
}
